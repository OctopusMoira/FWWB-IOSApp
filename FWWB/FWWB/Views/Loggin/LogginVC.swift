...
//  FWWB
//
//  Created by JingyaoTang on 2019/3/18.
//  Copyright © 2019年 fwwb. All rights reserved.
//
import UIKit
import SnapKit

class LogginVC: UIViewController, UITextFieldDelegate {
    ...
    var paramdict:NSMutableDictionary = ["username":"","password":""] //
    ...
    override func viewDidLoad() {
        ...
        //视图背景色
        self.view.backgroundColor = cBubbleColorHeavy
        ...
        self.formView.layer.cornerRadius = 6
        ...
        imgLock1.image = UIImage(named:"hallowHeart")
        
        //密码图标
        let imgLock2 =  UIImageView(frame:CGRect(x: 11, y: 11, width: 22, height: 22))
        imgLock2.image = UIImage(named:"fullHeart")
        ...
        self.confirmButton.setTitleColor(cBubbleColorHeavy,
                                         for: UIControl.State())
    ...
    func textFieldShouldEndEditing(_ textField: UITextField) -> Bool {
        ...
        switch tag {
        case 100:
            paramdict["username"] = textField.text!
            self.txtPwd.becomeFirstResponder()
        case 101:
            paramdict["password"] = textField.text!
            loginConfrim()
        default:
    ...
    @objc func loginConfrim(){
        ...
        //视图约束恢复初始设置
        let loginAPIrecognizer = LoginAPI()
        let message = loginAPIrecognizer.recognizeLogin(url:loginAPIrecognizer.recognize_url ,data: self.paramdict)
        if(message==nil||message=="error message"){
            print(message ?? "wrong")
            UIView.animate(withDuration: 0.5, animations: { () -> Void in
